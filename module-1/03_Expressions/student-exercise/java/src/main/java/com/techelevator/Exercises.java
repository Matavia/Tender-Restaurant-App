...
public class Exercises {
	...
	public boolean in3050(int a, int b) {
		boolean aBIn3040Range = (a >= 30 && a <= 40) && (b >= 30 && b <= 40);
		boolean aBIn4050Range = (a >= 40 && a <= 50) && (b >= 40 && b <= 50);

		return aBIn3040Range ^ aBIn4050Range;
	...
	public int max1020(int a, int b) {
		...
		boolean aIn1020Range = a >= 10 && a <= 20;
		boolean bIn1020Range = b >= 10 && b <= 20;
		boolean aAndBInRange = aIn1020Range && bIn1020Range;

		if ((aAndBInRange && a > b) || (aIn1020Range && !bIn1020Range)) {
			return a;
		}

		else if ((aAndBInRange && b > a) || (bIn1020Range && !aIn1020Range)) {
			return b;
		}
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		...
		boolean successfulCigarRange = cigars >= 40 && cigars <= 60;

		return successfulCigarRange || (isWeekend && cigars >= 40);
	}
	...
	public int dateFashion(int you, int date) {
		...
		int no = 0;
		int maybe = 1;
		boolean oneOfYouStylish = (you >= 8) || (date >= 8);
		boolean oneOfYouNotStylish = (you <= 2) || (date <= 2);

		if (oneOfYouStylish && !oneOfYouNotStylish) {
			return yes;

		} else if (oneOfYouNotStylish) {
			return no;
		}
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		...
		boolean playWeather = temp >= 60 && temp <= 90;
		boolean playWeatherInSummer = temp >= 60 && temp <= 100;

		return (playWeather && !isSummer) || (playWeatherInSummer && isSummer);
	...
	public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
		...
		double adjustedMealAmount = mealAmount;

		if (isBirthday) {
			adjustedMealAmount += 5.00;
		}

		boolean standardDesertMealAmount = adjustedMealAmount <= 10.00;
		boolean specialDesertMealAmount = adjustedMealAmount >= 10.00 && adjustedMealAmount <= 15.00;

		String standardDesert = "standard";
		String specialDesert = "special";
		String ginormousDesert = "ginormous";

		if (standardDesertMealAmount) {
			return standardDesert;
		}

		else if (specialDesertMealAmount) {
			return specialDesert;
		}

		else {
			return ginormousDesert;
		}
	}
	...
	public String alarmClock(int day, boolean vacation) {
		...
		String weekendAndVacayWeekdayAlarm = "10:00";
		String weekendVacayAlarm = "off";
		...
		else if ((weekend && !vacation) ^ (weekday && vacation)) {

			return weekendAndVacayWeekdayAlarm;
		}

		else {
			return weekendVacayAlarm;
		}
	}
	...
	// return true if n is 1 - 10 inclusive

	// if outsidemode is true, return true is number <= 1 || >= 10

	public boolean in1To10(int n, boolean outsideMode) {
		boolean inRange = n > 0 && n <= 10;
		boolean outsideModeRange = n <= 1 || n >= 10;

		return (inRange && !outsideMode) ^ (outsideModeRange && outsideMode);
	}
	...
	// special numbers multiples of 11 || or one more than a multiple of 11

	// return true if non neg number is special
	...
	// Return true if given non neg is 1 || 2 more than multiple of 20
	 ...
	 * → false
	 */
	// return true if non-neg is multiple of 3 or 5, not both
	...
	public boolean nearTen(int num) {
		boolean withinOne = (num + 1) % 10 == 0 || (num - 1) % 10 == 0;
		boolean withinTwo = (num + 2) % 10 == 0 || (num - 2) % 10 == 0;
		boolean multipleOfTen = num % 10 == 0;

		return (withinOne || withinTwo) || multipleOfTen;
	}
	...
	// extra lucky values are 13 between 19 | Return 19 in this case

	public int teenSum(int a, int b) {

		boolean teenValues = (a >= 13 && a <= 19) || (b >= 13 && b <= 19);
	...
	public boolean answerCell(boolean isMorning, boolean isMom, boolean isAsleep) {
		...
		return (isMorning && isMom && !isAsleep) || !isAsleep && !isMorning;
	}
	...
	// outcome of party 0 - bad / 1 - good / 2 - great

	// 1- good = if tea && candy >=5 /

	// 2 - great = if (tea * 2) >= candy || ( candy * 2 ) >= tea
	...
	// return true if 2 of numbers add together to make the other
	...
	// return true for increasing order
	// a < b && c > b && !equalOk

	// equalOk (a == b && c > b) || a < b && b == c;
	public boolean inOrderEqual(int a, int b, int c, boolean equalOk) {
		boolean equalNotOkay = (a < b && c > b) && !equalOk;
		boolean equalIncreasing = equalOk && (a == b && c > b) || (a < b && b == c);
		boolean allEqual = equalOk && (a == b && b == c);

		return (equalNotOkay || equalIncreasing) || allEqual;
	}
	...
	// if a == b, then b + c || b == c, then a + b ||

	public int loneSum(int a, int b, int c) {

		boolean allDuplicate = (a == b && b == c) && b == c ;
		boolean duplicateAB = a == b && !allDuplicate;
		boolean duplicateBC = b == c && !allDuplicate;
		boolean duplicateAC = a == c && !allDuplicate;

		if ((!allDuplicate && !duplicateAB) && (!duplicateBC && !duplicateAC)) {
			return a + b + c;
