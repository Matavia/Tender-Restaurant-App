...
SELECT name, population, surfacearea, AVG(lifeexpectancy)
FROM country
...
SELECT name, continent,lifeexpectancy, population, gnp
FROM country
...
SELECT (gnp/population) * 1.4389 AS GNP_Per_Capita_In_US_Dollars
FROM country
WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name AS country, (2021 - indepyear) AS years_since_independence 
FROM country
