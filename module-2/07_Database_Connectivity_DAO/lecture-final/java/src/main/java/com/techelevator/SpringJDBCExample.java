...
public class SpringJDBCExample {
	...
	public static void main(String[] args) {
		...
		//if craig is already inserted, we can't insert him again!
		 ...
		 * a new record.
		 */
		String sqlGetNextId = "SELECT nextval('seq_city_id') AS city_id";
		
		results = worldJdbcTemplate.queryForRowSet(sqlGetNextId);
		
		// normally with multiple results we'd have a while loop here
		// but there's only one result so we can just advance manually
		results.next(); // advances to the first row
		
		int id = results.getInt("city_id"); // returns the integer value of the first column (i.e. index 1)
		...
		String createNewCityReturning = "INSERT INTO city (name, countrycode, district, population) "+
										"VALUES (?,?,?,?) RETURNING id, name, countrycode, district, population";
		
		SqlRowSet cityResults = worldJdbcTemplate.queryForRowSet(createNewCityReturning, "Smallville-Earth2", "USA","Kansas",45001);
