...
-- (19 rows)
 ...
 SELECT (name || ',' || district) AS city_and_state, population
       FROM city
...
SELECT (name || ' ' || code) AS country_countrycode,region
FROM country
...
SELECT ((gnp * 1000000) / population) AS gnp_percaptiaoo
FROM country
WHERE population > 0
ORDER BY gnp_percaptia DESC;
...
SELECT MAX(population/surfacearea) AS highest_population_density
FROM country;
...
-- (smallest difference: 1.00, 1.00, "Ecuador")
 ...
 SELECT (gnpold - gnp) AS gnp_difference, name
 FROM country;
...
-- (45 rows)
SELECT COUNT(name) AS num_of_cities, district AS state
FROM city
WHERE countrycode = 'USA'
GROUP BY district
ORDER BY district;
...
SELECT  MAX(population) AS highest_city_population, countrycode
FROM city
